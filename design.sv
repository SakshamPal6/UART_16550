...
module regs_uart(
  ...
  */
  ...
  ///////// update dlsb if wr = 1 dlab = 1 and addr = 0
    ...
    begin
        if (rst)
            dl.dlsb <= 8'h01;  // ← Add default/safe value
        else if ( wr_i && addr_i == 3'b000 && csr.lcr.dlab == 1'b1)
            dl.dlsb <= din_i;
    end    
    ...
    begin
        if (rst)
            dl.dmsb <= 8'h00;  // ← Add default/safe value
        else if ( wr_i && addr_i == 3'b001 && csr.lcr.dlab == 1'b1)
            dl.dmsb <= din_i;
    end
  ...
  */
  ...
  if(rst)
  begin
  csr.lsr <= 8'h60; //// both fifo and shift register are empty thre = 1 , temt = 1  // 0110 0000
  end
  else
  begin
  ...
  csr.lsr.fe <=   rx_fe;
  csr.lsr.bi <=   rx_bi;
  csr.lsr.thre <=   tx_fifo_empty_i;
  csr.lsr.temt <=   tx_reg_empty_i;
  end
  end
...
module fifo_top(
  ...
  // ✅ EDGE DETECTION: Detect rising edge of pop_in
  logic pop_in_r;  // Register to store previous value
  logic pop_edge;  // Rising edge pulse

  always_ff @(posedge clk, posedge rst) begin
    if (rst)
      pop_in_r <= 1'b0;
    else
      pop_in_r <= pop_in;  // Capture previous value
  end
  ...
  always_ff @(posedge clk) begin
    ...
    always_ff @(posedge clk) begin
        ...
        rptr <= 4'd0;
        cnt  <= 4'd0;
        for(int i=0;i<16;i++) begin mem[i] <= 8'h00; end
        end else begin
            if (push) wptr <= (wptr == 4'd15) ? 4'd0 : wptr + 1'b1;
            if (pop) rptr <= ((rptr == 4'd15) ? '0 : rptr + 1'b1);
            unique case ({push, pop})
                2'b10:   cnt <= cnt + 1'b1;   // write only  → count up
                2'b01:   cnt <= cnt - 1'b1;  // read  only  → count down
                2'b11: cnt <= cnt;
                default: cnt <= cnt;                   
            endcase
    ...
    assign thre_trigger = (cnt >= threshold);
    assign underrun = pop_in && empty;
    assign overrun = push_in && full;
...
module all_mod(
 ...
 /////////////UART Registers
  ...
  uart_tx_top uart_tx_inst (
      ...
      .pen (csr.lcr.pen),
      .thre (tx_fifo_empty),             // to be connected to thresh trigger
      .stb (csr.lcr.stb),
      ...
      .wls (csr.lcr.wls),
      .pop (tx_fifo_pop),
      .sreg_empty (tx_reg_empty), ///sreg empty ier
      .tx (tx)
  );
